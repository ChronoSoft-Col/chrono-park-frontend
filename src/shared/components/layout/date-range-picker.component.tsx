...
export function DateRangePickerComponent() {
  ...
  const updateUrlParams = useCallback(
    (range: DateRange | undefined) => {
      ...
      // Reset a page 1 cuando cambie el rango
      params.delete("page");
  ...
  const formatDateRange = () => {
    if (!dateRange?.from) return "Seleccionar rango de fechas";
    if (!dateRange.to)
  ...
  const triggerClassName = cn(
    ...
    isMobile ? "w-[190px] overflow-hidden whitespace-nowrap" : "min-w-[280px]",
    !dateRange?.from && "text-muted-foreground"
  );

  const isClearDisabled = !dateRange?.from && !dateRange?.to;

  return (
    <div className="flex items-center gap-2">
      <ChronoPopover open={isPickerOpen} onOpenChange={setIsPickerOpen}>
        <ChronoPopoverTrigger asChild>
          <ChronoButton type="button" variant="outline" className={triggerClassName}>
