...
type Props = {
  onSubmit: (data: CreateRoleForm, actionIds: string[]) => Promise<boolean>;
  onCancel?: () => void;
  actionGroups: IActionGroupEntity[];
};

export function CreateRoleFormComponent({
  onSubmit,
  onCancel,
  actionGroups,
}: Props) {
  ...
  const form = useForm<CreateRoleForm>({
    resolver: zodResolver(CreateRoleSchema) as Resolver<CreateRoleForm>,
    mode: "onChange",
    defaultValues: {
      name: "",
      description: "",
