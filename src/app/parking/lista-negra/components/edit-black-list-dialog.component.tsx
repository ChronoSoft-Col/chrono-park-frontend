...
import { updateBlackListAction } from "../actions/update-black-list.action";
import { UseDialogContext } from "@/shared/context/dialog.context";
import type { IBlackListEntity, IUpdateBlackListParamsEntity } from "@/server/domain";
import {
  UpdateBlackListForm,
  UpdateBlackListSchema,
} from "@/src/shared/schemas/parking/black-list.schema";
...
export function EditBlackListDialogContent({ item }: { item: IBlackListEntity }) {
  const router = useRouter();
  const { closeDialog } = UseDialogContext();

  const form = useForm<UpdateBlackListForm>({
    resolver: zodResolver(UpdateBlackListSchema) as Resolver<UpdateBlackListForm>,
    mode: "onChange",
    defaultValues: {
      reason: item.reason ?? "",
  ...
  const handleFormSubmit = handleSubmit(async (data) => {
    const parsed = UpdateBlackListSchema.safeParse(data);
    if (!parsed.success) {
    ...
    try {
      const payload: IUpdateBlackListParamsEntity = {
        reason: parsed.data.reason || undefined,
        vehicleId: parsed.data.vehicleId || undefined,
        customerId: parsed.data.customerId || undefined,
      };

      const result = await updateBlackListAction(item.id, payload);
      ...
      console.error("Error updating black list entry:", error);
        ...
        <Controller
          ...
          render={({ field, fieldState }) => (
            <ChronoField data-invalid={fieldState.invalid} className={`${fieldContainerClasses} md:col-span-2`}>
              ...
              <ChronoInput {...field} id="reason" placeholder="Ej: Deuda pendiente, Comportamiento inadecuado" className="mt-1" />
              {fieldState.invalid && <ChronoFieldError errors={[fieldState.error]} />}
            </ChronoField>
          )}
        />
