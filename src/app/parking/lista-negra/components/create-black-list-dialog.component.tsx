...
import { createBlackListAction } from "../actions/create-black-list.action";
import { UseDialogContext } from "@/shared/context/dialog.context";
import type { ICreateBlackListParamsEntity } from "@/server/domain";
import {
  CreateBlackListForm,
...
export function CreateBlackListDialogContent() {
  const router = useRouter();
  const { closeDialog } = UseDialogContext();

  const form = useForm<CreateBlackListForm>({
    resolver: zodResolver(CreateBlackListSchema) as Resolver<CreateBlackListForm>,
    mode: "onChange",
    defaultValues: {
      vehicleId: "",
      customerId: "",
  ...
  const handleFormSubmit = handleSubmit(async (data) => {
    const parsed = CreateBlackListSchema.safeParse(data);
    if (!parsed.success) {
    ...
    try {
      const payload: ICreateBlackListParamsEntity = {
        vehicleId: parsed.data.vehicleId,
        reason: parsed.data.reason,
        customerId: parsed.data.customerId || undefined,
      };

      const result = await createBlackListAction(payload);
      ...
      console.error("Error creating black list entry:", error);
